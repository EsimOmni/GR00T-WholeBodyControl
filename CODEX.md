...
- Read the local context before editing. Prefer `rg` / `rg --files` for search.
- Keep changes scoped to the requested behavior and the relevant subproject.
- Do not revert user changes or unrelated dirty worktree state.
- Use patch-style edits for manual file changes.
- Do not run destructive commands such as `git reset --hard`, recursive deletes, or checkout-based reverts unless the user explicitly requests them.
- If `git status` or similar commands fail because Git LFS filters cannot access `.git/lfs/tmp`, report it and continue with non-mutating inspection.
